...
export const timer = {
  ...
  set tomatoDuration(newTomatoDuration) {
    ...
    if (this.current === 'tomato') {
      ...
      if (
        passed < newTomatoDuration ||
        (passed === newTomatoDuration && this.seconds >= 0)
      ) {
  ...
  set shortDuration(newShortDuration) {
    ...
    if (this.current === 'short') {
      let passed = this.duration - this.minutes;
      if (
        passed < newShortDuration ||
        (passed === newShortDuration && this.seconds > 0)
      ) {
  ...
  set longDuration(newLongDuration) {
    ...
    if (this.current === 'long') {
      let passed = this.duration - this.minutes;
      if (
        passed < newLongDuration ||
        (passed === newLongDuration && this.seconds > 0)
      ) {
  ...
  start() {
    if (this.paused === true) {
      ...
      document
        .querySelector('.timer')
        .querySelector('.start')
        .querySelector('button').textContent = 'Pause';
    }
  },
  ...
  pause() {
    if (this.paused === false) {
      ...
      document
        .querySelector('.timer')
        .querySelector('.start')
        .querySelector('button').textContent = 'Start';
    }
  },
  ...
  finishInterval() {
    ...
    if (this.current === 'tomato') {
      ...
      if (this.currentTask) {
        ...
        taskList.changeTomatoesDone(
          this.currentTask.name,
          this.currentTask.tomatoesDone + 1,
          position,
        );
        if (
          ...
          if (this.autoSwitchTasks === true) {
            ...
            if (unfinishedTasks.length > 0) {
              let inFront = unfinishedTasks.filter((fullName) => {
                let currentIndex = taskList.taskList.indexOf(
                  this.currentTask.fullName,
                );
                return taskList.taskList.indexOf(fullName) > currentIndex
                  ? true
                  : false;
              });
              if (inFront.length > 0) {
  ...
  switchTask(name, position) {
    if (taskList.doesTaskExist(name)) {
      if (taskList.areThereDuplicates(name)) {
        if (position === undefined) {
          ...
          filteredTasks.forEach((fullName, index) => {
            ...
            filteredTasksString += `, created: ${format(
              taskList[fullName].created,
              'HH:mm:ss dd/MM/y',
            )}`;
            filteredTasksString += '.';
          });
      ...
      let newCurrentTask = document.querySelector(
        `#${this.currentTask.fullName.replaceAll(' ', '')}`,
      );
      newCurrentTask.classList.toggle('current-task');
    } else {
      console.error(
        "There's no such task. Provide a valid task name in the string format.",
      );
    }
  },
};
